...
func (v *Vpork) Checkin() error {
	...
	msg := fmt.Sprintf("账号 %s %s 签到成功 %s 剩余流量%s ", v.User.UserName, v.ConfigName, checkin.Get("msg").String(), checkin.Get("trafficInfo.unUsedTraffic").String())
	v.MsgPush(msg)
	return nil
}
...
// MsgPush 消息推送处理接口
func (v *Vpork) MsgPush(msg string) {
	log.Infof("push info %+v", v.Qqpush)
	// 优先推送 独立配置
